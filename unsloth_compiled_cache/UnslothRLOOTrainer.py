                ...
                with profiling_context(self, "vLLM.generate"):
                    if is_conversational({"prompt": prompts[0]}):
                        all_outputs = self.llm.chat(
                            ...
                            use_tqdm=False,
                            chat_template_kwargs=self.chat_template_kwargs, lora_request = self.model.load_lora('rloo_trainer_lora_model_' + (os.environ.get('CUDA_VISIBLE_DEVICES', '0').replace(',','')), load_tensors = True))
                    else:
