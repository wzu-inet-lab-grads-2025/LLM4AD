...
DRY_RUN="${DRY_RUN:-0}"
DISABLE_LORA_ROLLBACK="${DISABLE_LORA_ROLLBACK:-0}"
...
RUN_ID_PREFIX="${RUN_ID_PREFIX:-adaptive_op_plus_collapse_np1_TSP${TSP_LABEL}}"
...
EOH_RL_RUNTIME_OVERRIDES_JSON="$(
  ...
  DISABLE_LORA_ROLLBACK="${DISABLE_LORA_ROLLBACK}" \
...
forced={
    "eoh_rl": {
        ...
        "sft": {
            ...
            "load_lora_path": os.environ["SFT_LOAD_LORA_PATH"],
        },
        "grpo": {
        ...
        "tsp": {
            "default_scale": os.environ["TSP_SCALE"],
        },
    }
}
if str(os.environ.get("DISABLE_LORA_ROLLBACK", "0")).strip().lower() in {"1", "true", "yes", "on"}:
    forced["eoh_rl"].setdefault("grpo", {}).setdefault("lora_gate", {})["enabled"] = False
...
def merge(base, overrides):
    ...
    for key,value in (overrides or {}).items():
        result[key]=merge(result.get(key, {}), value) if isinstance(value,dict) and isinstance(result.get(key),dict) else value
    return result
text=os.environ.get("EOH_RL_RUNTIME_OVERRIDES_JSON","").strip(); root=merge(root,json.loads(text)) if text else root; data=root["eoh_rl"]; tsp=data["tsp"]; scale=tsp["default_scale"]; scale_cfg=tsp["scales"][scale]; evo=data["evolution"]; grpo=data["grpo"]; sft=data.get("sft",{}); mode=str(sft.get("mode","cold_start")); lora="<none/cold_start>" if mode=="cold_start" else (sft.get("load_lora_path") or os.path.join(str(sft.get("output_dir","")),"lora_sft")); gate=grpo.get("lora_gate",{}).get("enabled", str(grpo.get("deploy_policy","gated")).lower()=="gated"); print(data["inference_gpus"][0]); print(data["inference_ports"][0]); print(grpo["vllm_group_port"]); print(evo["max_sample_nums"]); print(evo["max_generations"]); print(grpo["num_generations"]); print(evo["pop_size"]); print(tsp["task_name"]); print(scale); print(scale_cfg["problem_size"]); print(scale_cfg["n_instance"]); print(mode); print(lora); print(str(data.get("adaptive_operator",{}).get("enabled",False)).lower()); print(str(data.get("collapse",{}).get("enabled",False)).lower()); print(str(gate).lower())' "${CONFIG_PATH}")
