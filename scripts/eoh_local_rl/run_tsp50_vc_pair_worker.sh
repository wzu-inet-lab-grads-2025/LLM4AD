...
run_variant() {
  local variant="$1" seed="$2" enabled reward_mode lr save_lora
  case "${variant}" in
    b0_fixed) enabled=0; reward_mode=vc_pair; lr="" ;;
    vc_pair_reset) enabled=1; reward_mode=vc_pair; lr="" ;;
    vc_pair_lr0) enabled=1; reward_mode=vc_pair; lr=0.0 ;;
    aggregate_reset) enabled=1; reward_mode=aggregate; lr="" ;;
    validity_reset) enabled=1; reward_mode=validity_only; lr="" ;;
    shuffled_reset) enabled=1; reward_mode=performance_shuffled; lr="" ;;
