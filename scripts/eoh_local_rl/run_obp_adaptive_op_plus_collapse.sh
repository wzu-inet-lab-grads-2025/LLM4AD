...
SOURCE_CONFIG="${SOURCE_CONFIG:-${PROJECT_ROOT}/configs/run_eoh_local_rl/eoh_local_rl_obp.yaml}"
RUN_ROOT_BASE="${RUN_ROOT_BASE:-${PROJECT_ROOT}/logs/OBP/eoh_local_rl}"
...
if [ ! -f "${PROJECT_ROOT}/example/run_eoh/obp/run_eoh_local_rl.py" ]; then
...
def merge(base, overrides):
    ...
    for key,value in (overrides or {}).items():
        result[key]=merge(result.get(key, {}), value) if isinstance(value,dict) and isinstance(result.get(key),dict) else value
    return result
text=os.environ.get("EOH_RL_RUNTIME_OVERRIDES_JSON","").strip(); root=merge(root,json.loads(text)) if text else root; data=root["eoh_rl"]; obp=data["obp"]; scale=obp["default_scale"]; scale_cfg=obp["scales"][scale]; evo=data["evolution"]; grpo=data["grpo"]; sft=data.get("sft",{}); mode=str(sft.get("mode","cold_start")); lora="<none/cold_start>" if mode=="cold_start" else (sft.get("load_lora_path") or os.path.join(str(sft.get("output_dir","")),"lora_sft")); gate=grpo.get("lora_gate",{}).get("enabled", str(grpo.get("deploy_policy","gated")).lower()=="gated"); print(data["inference_gpus"][0]); print(data["inference_ports"][0]); print(grpo["vllm_group_port"]); print(evo["max_sample_nums"]); print(evo["max_generations"]); print(grpo["num_generations"]); print(evo["pop_size"]); print(obp["task_name"]); print(scale); print(scale_cfg["n_items"]); print(scale_cfg["n_instances"]); print(mode); print(lora); print(str(data.get("adaptive_operator",{}).get("enabled",False)).lower()); print(str(data.get("collapse",{}).get("enabled",False)).lower()); print(str(gate).lower())' "${CONFIG_PATH}")
...
N_ITEMS="${CFG_VALUES[9]}"
N_INSTANCES="${CFG_VALUES[10]}"
...
for i in $(seq 1 "${RUNS_PER_VARIANT}"); do
  RUN_ID="adaptive_op_plus_collapse_np1_OBP_run${i}_${RUN_ROOT_TS}"
  ...
  if [ "${DRY_RUN}" = "1" ]; then
    ...
    echo "task=${TASK_NAME} scale=${SCALE_NAME} n_items=${N_ITEMS} n_instances=${N_INSTANCES} sft_mode=${SFT_MODE} sft_lora=${SFT_LORA_PATH}"
    echo "config=${CONFIG_PATH}"
    continue
  fi
  EOH_RL_CONFIG_PATH="${CONFIG_PATH}" EOH_RL_RUN_ID="${RUN_ID}" EOH_RL_RUN_LOG_DIR="${RUN_LOG_DIR}" PYTHONUNBUFFERED=1 "${PYTHON}" "${PROJECT_ROOT}/example/run_eoh/obp/run_eoh_local_rl.py"
done
