...
LORA_PATH="/home/yuanyilun/projects/LLM4AD/data/SFT/tsp/medium-100/lora_sft"
RUN_STAMP="${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)}"
[ -d "${LORA_PATH}" ] || { echo "[ERROR] Missing SFT LoRA: ${LORA_PATH}" >&2; exit 1; }

for repeat in 1 2 3; do
  EXPERIMENT_ID=tsp100_vc_pair_500_3repeat RUN_BATCH_ID="${RUN_STAMP}" RUN_STAMP="${RUN_STAMP}" \
  RUN_ID_PREFIX="tsp100_vc_pair_persistent_TSP100_seed42_repeat${repeat}" \
  VARIANT=vc_pair_persistent ENABLE_GRPO=1 SEED=42 INITIAL_POPULATION_PATH= \
  TARGET_GPU=1 TARGET_PORT=22002 TARGET_GROUP_PORT=51214 \
  ...
  "${SCRIPT_DIR}/run_tsp100_eoh_rl.sh"
done
