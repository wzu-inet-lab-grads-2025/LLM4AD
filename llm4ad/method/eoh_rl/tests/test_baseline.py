...
from llm4ad.method.eoh_rl.controlled_eval import build_query_bank, load_checkpoint_parents, same_valid_summaries
...
class _Evaluator:
    def evaluate_program_with_profile(self, _program):
        return {"score": 1.0, "performance_profile": [1.0]}
...
def test_grpo_math_defaults_are_explicit():
    ...
    assert reward["pair_se_multiplier"] == 1.0
    assert reward["pair_margin"] == 1.0e-4
...
def test_event_funnel_has_one_definition_of_validity():
    events = [
        {"parse_success": False},
        {"parse_success": True, "evaluation_attempted": True},
        {
            "parse_success": True,
    ...
    assert funnel["contract_valid_count"] == 2
    assert funnel["evaluation_count"] == 2
    assert funnel["execution_valid_count"] == 1
...
def test_vc_pair_statistics_and_reward():
    positive = compare_profiles([2.0, 3.0], [1.0, 2.0], minimize=False, se_multiplier=1.0, margin=1.0e-4)
    neutral = compare_profiles([0.9, 2.1], [1.0, 2.0], minimize=False, se_multiplier=1.0, margin=0.0)
    negative = compare_profiles([2.0, 3.0], [1.0, 2.0], minimize=True, se_multiplier=1.0, margin=1.0e-4)
    ...
    with pytest.raises(ValueError, match="length mismatch"):
        compare_profiles([1.0], [1.0, 2.0], minimize=False, se_multiplier=1.0, margin=0.0)
    ...
    assert callback([record["messages"]], [_COMPLETION]) == [1.0]
    event = callback.events()[0]
    assert event["paired_outcome"] == "positive"
    assert event["paired_n"] == 1
...
def test_disabled_grpo_rolls_out_without_training():
    ...
    policy.draw_samples = lambda _prompt, n: [_COMPLETION] * n
...
def test_fixed_bank_and_same_valid_are_search_free():
    ...
    common = {"parse_success": True, "exec_success": True, "validity": True}
    ...
    assert summaries["a"]["eligible_count"] == summaries["b"]["eligible_count"] == 1
