...
class PartEvo:
    def __init__(
        ...
        **kwargs,
        ...
        self._initial_sample_nums_max = max(initial_sample_nums_max, 2 * self._pop_size)
    ...
    def _extend_init_population(self, tid=0, *args, **kwargs):
        ...
        try:
            current_population = (
                self._pool.population.copy() + self._pool.next_pop.copy()
            )
            current_feasible_population = [
                func for func in current_population if func.score is not None
            ]
            messages = PartEvoPrompt.get_prompt_batch_init(
    ...
    def init_from_local_algo_base(self):
        ...
        for seed_individual in seeds:
            ...
            program = TextFunctionProgramConverter.function_to_program(
                seed_str, self._template_program
            )
            ...
            if evaluation_return is not None:
                if isinstance(evaluation_return, (float, int)):
                    ...
                    func.all_ins_performance = evaluation_return.get(
                        "all_ins_performance", None
                    )
                    func.list_performance = evaluation_return.get(
                        "list_performance", None
                    )
            ...
            if self._posttrain_runtime is not None:
                ...
                self._posttrain_runtime.set_operator(operator)
                self._posttrain_runtime.set_parents([])
                self._posttrain_runtime.set_cluster(None)
                self._posttrain_runtime.set_generation(self._pool.generation)
            if self._posttrain_adapter is not None:
                self._posttrain_adapter.after_evaluate(
                    ...
                    sample_time=None,
                    eval_time=eval_time,
                    observation=getattr(func, "list_performance", None),
                    provenance={
                        "source": "seed_load",
                        "seed_path": self.local_algo_base,
                    },
                )
            ...
            if self._posttrain_adapter is not None:
                self._posttrain_adapter.after_register(
                    function=func, population=self._pool
                )
    ...
    def _sample_evaluate_register(
        ...
        phase = "init" if operator_name in {"init", "load"} else "search"
        if self._posttrain_runtime is not None:
            self._posttrain_runtime.begin_sample()
            self._posttrain_runtime.set_phase(phase)
            self._posttrain_runtime.set_operator(operator_name or None)
            self._posttrain_runtime.set_parents(parent_number)
            self._posttrain_runtime.set_cluster(from_which_cluster)
            self._posttrain_runtime.set_generation(self._pool.generation)
        if self._posttrain_adapter is not None:
            self._posttrain_adapter.before_sample(
                prompt=prompt,
                messages=messages,
                operator_name=operator_name,
                parent_number=parent_number,
                from_which_cluster=from_which_cluster,
                reflection=reflction,
            )
        thought, func, response = self._sampler.get_thought_and_function(
            prompt, image_prompt, messages
        )
        ...
        # register to the population
        self._pool.register_function(
            offspring=func, from_which_cluster=from_which_cluster
        )
        if self._posttrain_adapter is not None:
            self._posttrain_adapter.after_register(function=func, population=self._pool)
    ...
    def _continue_loop(self) -> bool:
        ...
        if self._max_sample_nums is None:
            ...
            return (
                self._pool.generation < self._max_generations
    ...
    def _partevo_multi_threaded_sampling(self, tid=0, *args, **kwargs):
        ...
        while self._continue_loop() and self._tot_sample_nums < target_samples:
            try:
                ...
                parent_candidates, operator_name, working_cluster_id = (
                    self._pool.select_parent()
                )
                parent_ids = [parent.sample_num for parent in parent_candidates]

                if operator_name == "error":
                    print(
                        f"\033[93m[Thread-{tid}] ❌ Warning: Parent selection failed. Please investigate.\033[0m"
                    )
                    continue
                ...
                if operator_name == "re":
                    ...
                    reflection_prompt_messages = PartEvoPrompt.get_prompt_reflection(
                        self._task_description_str,
                        primary_parent,
                        self._function_to_evolve,
                    )
                    generated_reflection = self._sampler.get_reflection(
                        prompt="", messages=reflection_prompt_messages
                    )
                    operator_messages = PartEvoPrompt.get_prompt_re(
                        self._task_description_str,
                        primary_parent,
                        self._function_to_evolve,
                    ...
                    if self._debug_mode:
                        print(
                            f"RE Prompt: {self.messages_to_string(operator_messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                ...
                # --- Operator logic: SE (Summary-based Evolution) ---
                    ...
                    (
                        current_summary,
                        requires_summary_update,
                        summary_context_samples,
                    ) = self._pool.external_archive.fetch_summary_context()
                    ...
                    if requires_summary_update and summary_context_samples:
                        ...
                        generated_summary = self._sampler.get_summary(
                            prompt="", messages=summary_prompt_messages
                        )
                        self._pool.external_archive.update_global_summary(
                            generated_summary
                        )
                        if generated_summary and generated_summary.strip():
                            current_summary = generated_summary
                            print(
                                f"\033[92m[Thread-{tid}] 🎉 Successfully generated a valid global summary.\033[0m"
                            )
                        else:
                            print(
                    ...
                    operator_messages = PartEvoPrompt.get_prompt_se(
                        self._task_description_str,
                        primary_parent,
                        self._function_to_evolve,
                        current_summary,
                    ...
                    if self._debug_mode:
                        print(
                            f"SE Prompt: {self.messages_to_string(operator_messages)}"
                        )
                ...
                # --- Operator logic: CN (Crossover between Niches) ---
                    ...
                    operator_messages = PartEvoPrompt.get_prompt_cn(
                        self._task_description_str,
                        parent_candidates,
                        self._function_to_evolve,
                    )
                    if self._debug_mode:
                        print(
                            f"CN Prompt: {self.messages_to_string(operator_messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        messages=operator_messages,
                ...
                # --- Operator logic: LGE (Local and Global guided Evolution ) ---
                    ...
                    operator_messages = PartEvoPrompt.get_prompt_lge(
                        self._task_description_str,
                        parent_candidates,
                        self._function_to_evolve,
                    )
                    if self._debug_mode:
                        print(
                            f"LGE Prompt: {self.messages_to_string(operator_messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        messages=operator_messages,
                    ...
                    raise Exception(
                        "ERROR: The input operators are not supported at the moment. Please check !!!!!"
                    )
    ...
    def init_using_llms(self):
        batch_num = 0
        while (
            len(self._pool.population) + len(self._pool.next_pop) < self._pop_size
            and self._tot_sample_nums <= self._initial_sample_nums_max
        ):  # 当被注册的个数还没有超过pop_size初始种群要求大小时
            batch_num += 1
    ...
    def using_flow(self, worst_case_percent=10, top_k=None):
        ...
        if top_k is not None and isinstance(top_k, int) and top_k > 0:
            ...
            try:
                ...
                print(
                    f"   -> Reduced population from {original_size} to {len(trained_data)}."
                )
            except Exception as e:
                print(
                    f"   -> ⚠️ Warning: Could not sort by 'score'. Using original order. Error: {e}"
                )

        using_time_start = time.time()
        print(
            "💪 [Brute Force Mode] Evaluating selected algorithms on each instance..."
        )
        ...
        for instance_id in ins_to_be_solve_id_set:
            ...
            for i, algo_json in enumerate(trained_data):
                print(
                    f"  -> Testing algorithm {i + 1}/{len(trained_data)}...", end="\r"
                )
                try:
                    # Parse algorithm code and execute secure evaluation
                    program = TextFunctionProgramConverter.function_to_program(
                        algo_json["function"], self._template_program
                    )
                    func = TextFunctionProgramConverter.text_to_function(str(program))

                    score_images_dict = self._evaluator._evaluate(
                        str(program),
                        func.name,
                    ...
                    score = (
                        score_images_dict.get("all_ins_performance", {})
                        .get(instance_id, {})
                        .get("score", float("-inf"))
                    )
                    ...
                    if score is not None and score > best_score_for_instance:
                        ...
                        best_perf_for_instance = score_images_dict.get(
                            "all_ins_performance", {}
                        )[instance_id]
                except Exception as e:
                    print(
            ...
            if best_algo_for_instance:
                print(f"   -> ✅ Best score found: {best_score_for_instance:.4f}")
                ...
                print(
                    f"   -> ⚠️ Warning: No algorithm produced a valid score for instance {instance_id}."
                )
        ...
        if valid_scores:
            ...
            final_results["average_score_of_all_instances"] = sum(valid_scores) / len(
                valid_scores
            )
        else:
        ...
        worst_avg_score = (
            sum(worst_scores_values) / len(worst_scores_values)
            if worst_scores_values
            else None
        )
