...
class MLES:
        ...
        if "m1_text" in self.operators:
            if hasattr(self.evaluation_object, "non_image_representation_explanation"):
                self._information_discription = (
                    self.evaluation_object.non_image_representation_explanation
                )
            else:
                raise ValueError(
        ...
        for seed_individual in seeds:
            ...
            if self._posttrain_adapter is not None:
                self._posttrain_adapter.after_evaluate(
                    ...
                    sample_time=0,
                    eval_time=eval_time,
                    observation=getattr(func, "observation", None),
                    image64=getattr(func, "image64", None),
                    provenance={"source": "seed_load", "seed_path": self.seed_path},
                )
    ...
    def _sample_evaluate_register(
        ...
        phase = (
            "init" if operator_name in {"Initialization", "load", "init"} else "search"
        )
        if self._posttrain_runtime is not None:
            ...
            self._posttrain_runtime.set_generation(self._population.generation)
            self._posttrain_runtime.set_extra(
                modality="multimodal" if messages is not None else "text",
            )
        if self._posttrain_adapter is not None:
        ...
        while self._continue_loop():
            try:
                ...
                if operator == "e1_advanced":
                    ...
                    parents_pop_register_number = [
                        ind.pop_register_number for ind in indivs
                    ]
                    messages = MLESPrompt.get_prompt_e1_advanced(
                    ...
                    prompt = MLESPrompt.get_prompt_e1(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                ...
                # get a new func using e2
                    ...
                    prompt = MLESPrompt.get_prompt_e2(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                ...
                # get a new func using e2
                    ...
                    messages = MLESPrompt.get_prompt_e2_advanced(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"E2_advanced Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        messages=messages,
                ...
                # get a new func using e2 Multimodal
                    ...
                    messages = MLESPrompt.get_prompt_e2_M(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"E2 Multimodal Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                ...
                # get a new func using m1
                    ...
                    messages = MLESPrompt.get_prompt_m1(
                ...
                # get a new func using m2
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m2(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                ...
                # get a new func using m1_Multimodal
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m1_M(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"M1_Multimodel Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m1_M_text_info(
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m2_M(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"M2_Multimodel Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                ...
                # no figure itself
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_image_description(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    description, response = self._sampler.get_image_description(
                        prompt="", image64s=None, messages=messages
                    )
                    messages = MLESPrompt.get_prompt_m1_M_image_description(
                        self._task_description_str,
                        indiv,
                        self._function_to_evolve,
                        description,
                    ...
                    if self._debug_mode:
                        ...
                        print(
                            f"m1_image_describtion Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                    ...
                    messages = MLESPrompt.get_prompt_m2_M_image_description(
                        self._task_description_str,
                        indiv,
                        self._function_to_evolve,
                        description,
                    ...
                    if self._debug_mode:
                        ...
                        print("Description response:", response)
                        print(
                            f"m2_image_describtion Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m1_M_only_image(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"M1_only_image_Multimodel Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                ...
                # Variants that without the "thought" of the algorithm during the algorithm generation.
                    ...
                    prompt = MLESPrompt.get_prompt_e1_nothought(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                    ...
                    prompt = MLESPrompt.get_prompt_e2_nothought(
                        self._task_description_str, indivs, self._function_to_evolve
                    )
                    if self._debug_mode:
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m1_M_nothought(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"M1_Multimodel_nothought Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                        messages=messages,
                        operator_name="m1_M_nothought",
                        parent_number=parents_pop_register_number,
                    )
                    if not self._continue_loop():
                        break
                    ...
                    indiv = indivs[0]
                    parents_pop_register_number = [indiv.pop_register_number]
                    messages = MLESPrompt.get_prompt_m2_M_nothought(
                        self._task_description_str, indiv, self._function_to_evolve
                    )
                    if self._debug_mode:
                        print(
                            f"M2_Multimodel_nothought Prompt: {self.messages_to_string(messages)}"
                        )
                    self._sample_evaluate_register(
                        prompt="",
                        image_prompt=None,
                        messages=messages,
                        operator_name="m2_M_nothought",
                        parent_number=parents_pop_register_number,
                    )
                    if not self._continue_loop():
                        break
    ...
    def _iteratively_init_population(self, tid=0):
        ...
        while self._population.generation == 0:
            try:
                ...
                prompt = MLESPrompt.get_prompt_i1(
