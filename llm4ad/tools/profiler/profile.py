...
class ProfilerBase:
    def __init__(
        ...
        **kwargs,
        ...
        self._process_start_time = datetime.now(_get_shanghai_timezone())
            ...
            if score is not None:
                for i in range(self._num_objs):
                    if score[i] > self._cur_best_program_score[i]:
                        ...
                        if not resume_mode:
                            self._write_json(
                                function, record_type="best", program=program
                            )
    ...
    def load_logfile(cls, logdir, valid_only=False) -> Tuple[List[str], List[float]]:
        ...
        sample_files = [
            f
            for f in all_files
            if f.startswith("samples_") and f != "samples_best.json"
        ]
