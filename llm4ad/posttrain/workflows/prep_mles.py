        ...
        return {
            "outcome": DatasetManifest(
                ...
                output_path=TrlChatFormatter().format(
                    ...
                    dataset_name=f"round_{self.runtime.round_id:04d}_mles_outcome",
                    context=context,
                ),
                config_digest="mles_outcome_v1",
            ),
            "process": DatasetManifest(
                ...
                output_path=ProcessJsonlFormatter().format(
                    process_examples,
                    output_dir=datasets_dir,
                    dataset_name=f"round_{self.runtime.round_id:04d}_mles_process",
                    context=context,
                ),
                config_digest="mles_process_v1",
            ),
            "verifier": DatasetManifest(
                ...
                output_path=VerifierJsonlFormatter().format(
                    verifier_examples,
                    output_dir=datasets_dir,
                    dataset_name=f"round_{self.runtime.round_id:04d}_mles_verifier",
                    context=context,
                ),
                config_digest="mles_verifier_v1",
            ),
        }
